"""Bug 3: Canonical action clustering and non-zero set overlap."""
...
from similarity.canonicalization import canonicalize_pair
...
def engine(tmp_path):
    ...
    return WorkflowSimilarityEngine(cfg, embedder), embedder


def test_canonicalize_remove_eliminate_same_cluster(engine):
    _, embedder = engine
    threshold = 0.7
    assert canonicalize_pair(
        "Remove damaged material",
        "Eliminate defective material",
        embedder,
        threshold,
    )


def test_set_overlap_nonzero_for_shared_weld_actions(engine):
    sim_engine, _ = engine
    query = Workflow(
        document="__query__",
        document_id="query",
        nodes=[
            WorkflowNode(node_id="q1", action="weld", object="repair", order=0),
            WorkflowNode(node_id="q2", action="remove", object=None, order=1),
            WorkflowNode(node_id="q3", action="grind", object=None, order=2),
            WorkflowNode(node_id="q4", action="preheat", object=None, order=3),
            WorkflowNode(node_id="q5", action="inspect", object=None, order=4),
        ],
    )
    candidate = Workflow(
    ...
    scores = sim_engine.score(query, candidate)
